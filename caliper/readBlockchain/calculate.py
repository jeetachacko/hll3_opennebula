...
home_dir = "/home/ubuntu/hll3_opennebula/caliper/readBlockchain/"
log_dir = "data"
...
def originator_sig():
    ...
    succrate = [[None for _ in range(2)] for _ in range(len(clients))]
    start = [[None for _ in range(2)] for _ in range(len(clients))]
    end = [[None for _ in range(2)] for _ in range(len(clients))]
    ...
    for k in range(len(clients)):
        ...
        for i in range(len(new_lines)):
            if i != 0 and new_lines[i][2]==clients[k]:
               if first == True:
                start[k] = arrow.get(new_lines[i][0]).datetime
                first = False           
               end[k] = arrow.get(new_lines[i][0]).datetime               
               txcount+=1
               if new_lines[i][6] != "VALID":
                failcount += 1
        succcount = txcount - failcount
        successrate = succcount/txcount
        succrate[k] = successrate
        ogrows[k] = [clients[k], txcount, succcount, failcount, successrate]
    
    print("Org1txcount:", ogrows[0][1])
    print("Org1successrate:", ogrows[0][4])
    print("Org2txcount:", ogrows[1][1])
    print("Org2successrate:", ogrows[1][4])
    TMOrg1 = (end[0] - start[0]).total_seconds()
    TMOrg2 = (end[1] - start[1]).total_seconds()
    TrOrg1 = ogrows[0][1]/TMOrg1
    TrOrg2 = ogrows[1][1]/TMOrg2
    
    SuccTrOrg1 = ogrows[0][2]/TMOrg1
    SuccTrOrg2 = ogrows[1][2]/TMOrg2
    
    succtrrate = [SuccTrOrg1, SuccTrOrg2]
    totalsucctrrate = totalsucccount/TM
    print("totalsucctrrate:", totalsucctrrate)

    print("==========================================================================================")
    print("Org1Tr:", TrOrg1)
    print("Org2Tr:", TrOrg2)
    print("Org1SuccTr:", SuccTrOrg1)
    print("Org2SuccTr:", SuccTrOrg2)
    print("Org1succcount:", ogrows[0][2])
    print("Org2succcount:", ogrows[1][2])
    succcountarr = [ogrows[0][2], ogrows[1][2]]
    jfi = (pow(sum(succcountarr), 2))/(len(clients) * sum(i*i for i in succcountarr))
    print("Jain's fairness index:", jfi)
    print("==========================================================================================")


    with open("/home/ubuntu/hll3_opennebula/caliper/blockreader-logs.txt", "w") as text_file:
        text_file.write("jfi=" + str(jfi) + "=" + "Org1txcount=" + str(ogrows[0][1]) + "=" + "Org1successrate=" + str(ogrows[0][4]) + "=" + "Org1SuccTr=" + str(SuccTrOrg1) + "=" + "Org1Tr=" + str(TrOrg1) + "=" + "Org2txcount=" + str(ogrows[1][1]) + "=" + "Org2successrate=" + str(ogrows[1][4]) + "=" + "Org2SuccTr=" + str(SuccTrOrg2) + "=" + "Org2Tr=" + str(TrOrg2) + "=" + "totalsucctrrate=" + str(totalsucctrrate) + "=" + "Org1succcount=" + str(ogrows[0][2]) + "=" + "Org2succcount=" + str(ogrows[1][2]) + "=")
...
def client_dist():
    ...
    for i in range(1, len(cnew_lines)):
        print("Client:", cnew_lines[i][0], "Number of transactions:", cnew_lines[i][1], "Number of success:", cnew_lines[i][2], "Number of failures:", cnew_lines[i][3], "Success rate:", cnew_lines[i][4])
