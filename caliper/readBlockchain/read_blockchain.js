...
4. kubectl port-forward svc/hlf-ca--org1 7054:7054 -n default &
5. kubectl port-forward svc/hlf-peer--org1--peer0 7051:7051 -n default &
7. Use new port numbers : http://localhost:7051

pkill -9 -f port-forward

kubectl expose service hlf-ca--org1 --port=7054 --target-port=7054 --name=caorg
kubectl expose service hlf-peer--org1--peer0 --port=7051 --target-port=7051 --name=peer0org1
*/
...
async function setClient() {
	...
	await client.initCredentialStores()
			...
			await client.setUserContext({username:'admin', password:'adminpw'})
				...
				for (let index = (blockchainheight - 30); index < blockchainheight; index++) {
