...
#Single tps - Uniform Workload - New rate controller
chaincode=generator
selectedtps=1000
num_entries=360
...
for i in $(seq 1 ${num_entries}); do
        configlines=( "    - label: common" "      txDuration: 10" "      rateControl:" "          type: /caliper-workload/ratecontroller.js" "          opts:" "            tps: ${selectedtps}" "      workload:" "        module: /caliper-workload/common.js" );
        printf '\n' >> /home/ubuntu/hll3_opennebula/caliper/benchmarks/$chaincode/config.yaml;
        printf '\n%s' "${configlines[@]}" >> /home/ubuntu/hll3_opennebula/caliper/benchmarks/$chaincode/config.yaml;
done
